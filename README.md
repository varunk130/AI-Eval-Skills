...
| **eval-generator** | Turns that plan into ready-to-run test cases with realistic inputs, expected outputs, and scoring criteria. Exports CSV in the Copilot Studio import format (works directly with Copilot Studio; easily adapted to any harness that accepts tabular test cases) | You have a plan (or even just an agent description) and need actual test data |
...
Or clone this repo and copy the contents of `skills/` into the location your agent reads from - `~/.claude/skills/` (or `.claude/skills/` in a project) for Claude Code, `.github/skills/` for GitHub Copilot.
